...
# Garde-fou peu coûteux : on ne traite qu'une vidéo à la fois. Ça n'a rien à voir avec
# le nombre de membres du serveur, mais ça évite que deux téléchargements/compressions
# tombent pile au même moment et cumulent leur pic de RAM.
...
def compress_video(input_path: str, output_path: str, target_size_bytes: int) -> bool:
    ...
    cmd = [
        ...
        '-b:v', str(video_bitrate), '-maxrate', str(int(video_bitrate * 1.2)),
...
def download_tiktok(url: str, out_dir: str):
    ...
    ydl_opts = {
        ...
        # On limite directement la résolution au téléchargement : inutile de tirer la meilleure
        # qualité puis de la recompresser à chaque fois, ça double le travail (et la RAM utilisée)
        # pour rien sur un petit serveur Render.
